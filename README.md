...
Download the HyperExecute CLI (concierge) corresponding to your host OS:
...
```bash
./concierge --config --verbose -i yaml/win/.hyperexecute_matrix.yaml
```

**Auto-split mode:**

```bash
./concierge --config --verbose -i yaml/win/.hyperexecute_autosplits.yaml
```
